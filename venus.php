...
$sql = "SELECT * FROM mainplanets WHERE PlanetName='Venus'";
...
$sql="select FactNumber1, FactNumber2, FactNumber3 FROM funfacts where PlanetName='Venus'";
$result = $conn->query($sql);
...
$sql="select NoOfMoons FROM moonsoftheplanets where NameOfThePlanet='Venus'";
$result = $conn->query($sql);
...
nav img{
	...
	.header1{
		...
		background-image:  url('VENUSMAIN.gif');
		height: 87vh;
		width: 98vh;
		left: 22%;
		text-align: center; 
...
<div  style="position: absolute; text-align: center; top: 16%; left: 36%;color: rgb(255, 255, 255);">
	<h1 style="font-family: Arial, Helvetica, sans-serif;  font-size: 80px;"><br> Venus</h1>
</div>
...
<div style="position: absolute; left: 38%; top: 72%; font-family: Arial, Helvetica, sans-serif; font-size: 20px;">
	<h1 style="color: blanchedalmond;font-family: Arial, Helvetica, sans-serif; "> Fun facts about Venus </h1>
	</div>
...
<br>
	...
	<a href="mercury.php">PREV PLANET</a>
	<a href="earth.php">NEXT PLANET</a>
</div>
</section>
