...
$sql = "SELECT * FROM mainplanets WHERE PlanetName='Saturn'";
...
$sql="select FactNumber1, FactNumber2, FactNumber3 FROM funfacts where PlanetName='Saturn'";
$result = $conn->query($sql);
...
$sql="select NoOfMoons FROM moonsoftheplanets where NameOfThePlanet='Saturn'";
$result = $conn->query($sql);
...
nav img{
	...
	.header1{
		...
		background-image:  url('SATURNMAIN.gif');
		height: 87vh;
		width: 98vh;
		left: 22%;
		text-align: center; 
...
<div  style="position: absolute; text-align: center; top: 16%; left: 36%;color: rgb(255, 255, 255);">
	<h1 style="font-family: Arial, Helvetica, sans-serif;  font-size: 80px;"><br> Saturn</h1>
</div>
...
<div style="position: absolute; left: 38%; top: 72%; font-family: Arial, Helvetica, sans-serif; font-size: 20px;">
	<h1 style="color: blanchedalmond;font-family: Arial, Helvetica, sans-serif; "> Fun facts about Saturn </h1>
	</div>
...
<br>
	...
	<a href="jupiter.php">PREV PLANET</a>
	<a href="neptune.php">NEXT PLANET</a>
</div>
</section>
