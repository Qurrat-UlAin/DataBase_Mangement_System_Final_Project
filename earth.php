...
$sql = "SELECT * FROM mainplanets WHERE PlanetName='Earth'";
...
$sql="select FactNumber1, FactNumber2, FactNumber3 FROM funfacts where PlanetName='Earth'";
$result = $conn->query($sql);
...
$sql="select NoOfMoons FROM moonsoftheplanets where NameOfThePlanet='Earth'";
$result = $conn->query($sql);
...
nav img{
	...
	.header1{
		...
		background-image:  url('EARTHMAIN.gif');
		height: 87vh;
		width: 98vh;
		left: 22%;
		text-align: center; 
...
<div  style="position: absolute; text-align: center; top: 16%; left: 36%;color: rgb(255, 255, 255);">
	<h1 style="font-family: Arial, Helvetica, sans-serif;  font-size: 80px;"><br> Earth</h1>
</div>
...
<div style="position: absolute; left: 38%; top: 72%; font-family: Arial, Helvetica, sans-serif; font-size: 20px;">
	<h1 style="color: blanchedalmond;font-family: Arial, Helvetica, sans-serif; "> Fun facts about Earth </h1>
	</div>
...
<br>
	...
	<a href="venus.php">PREV PLANET</a>
	<a href="mars.php">NEXT PLANET</a>
</div>
</section>
