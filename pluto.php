...
$sql = "SELECT * FROM otherbodies WHERE BodyName='Pluto'";
...
if ($result->num_rows > 0) {
 ...
 while($row = $result->fetch_assoc()) {
   ...
   $massOfTheBody=$row["MassOfTheBody"];
...
$sql="select FactNumber1, FactNumber2, FactNumber3 FROM funfacts where PlanetName='Pluto'";
$result = $conn->query($sql);
...
nav img{
	...
	.header1{
		...
		background-image:  url('PlutoMAIN.gif');
		height: 87vh;
		width: 98vh;
		left: 22%;
		text-align: center; 
...
<div  style="position: absolute; text-align: center; top: 16%; left: 36%;color: rgb(255, 255, 255);">
	<h1 style="font-family: Arial, Helvetica, sans-serif;  font-size: 80px;"><br> Pluto</h1>
</div>
<div style="width: 80%;position: relative; height: 300px; border: 3px solid blanchedalmond;text-align: center; left: 150px;border-radius: 20px; ">
	...
	<h1 style="color:blanchedalmond ; font-weight: lighter	; font-size: 30px;"><?php echo $massOfTheBody; ?>Kg</h1>
	</div> 
...
<div style="position: absolute; left: 38%; top: 72%; font-family: Arial, Helvetica, sans-serif; font-size: 20px;">
	<h1 style="color: blanchedalmond;font-family: Arial, Helvetica, sans-serif; "> Fun facts about Pluto </h1>
	</div>
...
<br>
	...
	<div style="position: absolute; bottom: 1%; left: 45%;  " class="TC" >
	<a href="243ida.php">NEXT PLANET</a>
</div>
</section>
